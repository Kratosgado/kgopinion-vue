...
const savePost = async (status: PostStatus) => {
  const { success, msg } = await editorStore.saveContent(status, auth.state.user?.id)
...
<template>
  <div
    ...
    <div class="flex items-center gap-2">
      <div class="btn-group">
        <button class="btn btn-sm" :class="{ 'btn-primary': currentView === 'editor' }" @click="changeView('editor')">
          Editor
        </button>
        <button class="btn btn-sm" :class="{ 'btn-primary': currentView === 'preview' }" @click="changeView('preview')">
          Preview
        </button>
        <button class="btn btn-sm" :class="{ 'btn-primary': currentView === 'code' }" @click="changeView('code')">
          HTML
        </button>
      </div>
