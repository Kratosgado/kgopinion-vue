  ...
  actions: {
    ...
    updateSeoScore() {
      ...
      if (analysis.keywordDensity > 0 && analysis.keywordDensity <= 2.5) {
        score += 15
      } else if (analysis.keywordDensity > 2.5 && analysis.keywordDensity <= 4) {
        score += 7.5
      }
      ...
      if (analysis.descriptionLength >= 120 && analysis.descriptionLength <= 155) {
        score += 15
      } else if (analysis.descriptionLength > 0 && analysis.descriptionLength < 120) {
        score += 7.5
      }
    ...
    generateImprovements() {
      ...
      if (!metaTitle) {
        ...
        improvements.push('Meta title is too long. Keep it under 60 characters')
      }

      if (!metaDescription) {
        improvements.push('Add a meta description for better SEO')
      } else if (analysis.descriptionLength < 120) {
        improvements.push('Meta description is too short. Aim for 120-155 characters')
      } else if (analysis.descriptionLength > 155) {
        improvements.push('Meta description is too long. Keep it under 155 characters')
      }

      if (focusKeyword && !analysis.keywordInTitle) {
        improvements.push(`Include the focus keyword "${focusKeyword}" in the title`)
      }

      if (focusKeyword && !analysis.keywordInFirstParagraph) {
        improvements.push(`Include the focus keyword "${focusKeyword}" in the first paragraph`)
      }

      if (focusKeyword && analysis.keywordDensity === 0) {
        improvements.push(`Include the focus keyword "${focusKeyword}" in your content`)
